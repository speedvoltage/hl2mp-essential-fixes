...
If you are using sv_pure 1 or above, make sure you have whitelisted the sounds by adding `sound\server_sounds\... any` in your **pure_server_whitelist.txt** or **pure_server_full.txt**

## The player markers show up as pink checkerboards

If you are using sv_pure 1 or above, make sure you have whitelisted the sprite by adding `materials\server_materials\... any` in your **pure_server_whitelist.txt** or **pure_server_full.txt**
...
### For Modified Files:
1. If you have a customized **pure_server_full.txt**, ensure these lines are added:  
```
sound\server_sounds\... any
materials\server_materials\... any
```
2. If you have a customized **skill.cfg**, verify `sk_suitcharger` is set to `30` to maintain the default value.
...
Usually, the compressed folder in the Release section contains the following files and directories:
```
- hl2
  - scripts
...
- Flashlight has a 0.25 second cooldown period to avoid spam.
- Live mp_teamplay updates.
- `mp_timelimit` updates now reset the time left e.g. if the server had `mp_timelimit` set to 15 and 5 minutes elapsed and the ConVar is updated to 20, the time left will be set to 20 minutes rather than the default behavior which would have been the new time limit minus the elapsed time (20 minutes - 5 minutes).
...
- Enhanced player visibility and sounds with equalizer.
...
- Added player markers in equalizer mode.

---

# Equalizer

**Equalizer** is a mode designed to enhance player visibility and improve audio cues during gameplay. Once activated, Equalizer enforces specific visual and audio settings that make the game fairer and easier to navigate. You can enable Equalizer once per map by typing `!e`, `!eq`, or `!equalizer` in the chat. Note that the `sv_equalizer_allow_toggle` ConVar must be enabled for these chat triggers to work.  
**Important:** Once activated, Equalizer cannot be disabled for the remainder of the map!

---

## Enhanced Player Visibility in Deathmatch (DM) and Team Deathmatch (TDM)

Equalizer forces all players to use the Combine Super Soldier model, which already has a bright white appearance to increase visibility. Player models cannot be changed while Equalizer is active. Additionally, players are color-coded by their teams:  

- **Rebels**: Default color is red.  
- **Combine**: Default color is green.  

You can customize these colors using RGB-based ConVars:  

- Rebels: `sv_equalizer_rebels_red`, `sv_equalizer_rebels_green`, `sv_equalizer_rebels_blue`.  
- Combine: `sv_equalizer_combine_red`, `sv_equalizer_combine_green`, `sv_equalizer_combine_blue`.  

![Player visibility example](https://github.com/user-attachments/assets/50ec3c16-bcb7-4eaf-81a1-be0a58e91875)

> **Note:** Some maps include a prefab that enhance player visibility further by making player models appear extremely bright. This behavior is achieved by dynamically adjusting the lighting origin of the player model if such a prefab is detected. On maps without this prefab, player models will retain their default lighting origin while still being color-coded.

---

## Improved Audio Cues

Equalizer also modifies footstep sounds to differentiate between allies and enemies:  

- **Your footsteps**: Always use citizen footstep sounds, regardless of your team.  
- **Teammates**: Always use citizen footstep sounds, regardless of team, when TDM is enabled.  
- **Enemies**: Always use Combine footstep sounds, regardless of team.  

In addition to these changes, footstep sound processing has been moved to the server side for enhanced control. This provides:  

1. Consistency across all players.  
2. Fine-tuned audio adjustments, such as volume and pitch.  

> **Note:** This server-side adjustment also fixes the broken footstep sounds introduced in the anniversary update.

Equalizer is designed to create a fairer and more accessible gameplay experience by improving both visibility and audio clarity.

---

# Player Markers (ESP)

Player markers, or ESP, are a feature designed for **Team Deathmatch**. These markers help players locate their teammates by displaying a marker above their models, visible even through walls.  
...
- **Teammate-only visibility**:  
  Player markers are visible only for your teammates and are disabled in Deathmatch mode.  

- **Customizability**:  
  Players can disable their markers by typing `!esp` in chat. This setting is saved for future sessions.  

This feature complements Equalizer but functions independently, providing additional team coordination benefits in TDM.

---

### Visual Examples:
![Example 1](https://github.com/user-attachments/assets/e4d32890-e08a-47e6-af33-a17ff486964a)  
![Example 2](https://github.com/user-attachments/assets/cf614759-87c5-4736-83fd-4ba7f17e8bd4)  
![Example 3](https://github.com/user-attachments/assets/a92ca329-d434-41a2-b8bf-d4aef09b7b20)  
![Example 4](https://github.com/user-attachments/assets/29eb38a4-9f3c-413d-8d44-5eddfa004fbb)  
![Example 5](https://github.com/user-attachments/assets/1ec1f8ad-112a-4a76-aba8-b1fdb4589b15)  
![Example 6](https://github.com/user-attachments/assets/31639f34-68da-46bb-8e72-17d5eba85e03)  
![Example 7](https://github.com/user-attachments/assets/957da90b-cf95-4300-8336-2bf8bc044c2e)
...
| `sv_chat_triggers`                           | Allows players to use chat triggers                                                                 |
...
| `sv_showmotd`                    			   | Shows Message of the Day (MOTD) when player connects                                                |
...
| `sv_soundfix`                 			   | Enables the sound fixes, allowing broken sounds to work again		                                 |
| `mp_dsp`                					   | Enables the ear ringing effect caused by explosions (it disables !dsp from being used as a player setting)          |
...
| `!dsp`                  | Toggles DSP effects (ear ringing) on or off              |
| `!esp`                  | Toggles player markers on or off              			 |
